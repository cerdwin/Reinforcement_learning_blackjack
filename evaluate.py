...
def evaluate(rewards: List[float]):
    ...
    print(simple_moving_average(rewards, 4000))
    ...
    plot_series(simple_moving_average(rewards, 4000), 'simple_moving_average.png')
    plot_series(exponential_moving_average(rewards, 0.001), 'exponential_moving_average.png')
...
def plot_series(arr, figname):

     plt.plot(arr, c="b")
     plt.xlabel('Episodes')
     plt.ylabel('Reward')
     plt.title('TD agent exp. moving average of rewards')
